...
local function loadLibrary()
    ...
    if not ok then
        error("[DS HUB] Erro ao baixar Hub.lua: " .. tostring(source))
    end

    local fn, compileError = loadstring(source)

    if not fn then
        error("[DS HUB] Erro ao compilar Hub.lua: " .. tostring(compileError))
    end
    ...
    if type(library) ~= "table" or type(library.Init) ~= "function" then
        error("[DS HUB] Hub.lua não retornou uma tabela válida com Init().")
    end
...
-- Correção do parâmetro da aba (Nome separado do Ícone)
...
local function saveEnabled(value)
    ...
    pcall(function()
        TeleportService:SetTeleportSetting(ENABLED_KEY, value == true)
    end)
end
...
local function killNPCs(radius)
    ...
    if not folder or not flowModule then return end
    ...
    for _, object in ipairs(folder:GetDescendants()) do
        if object:IsA("Humanoid") and object.Health > 0 then
            ...
            if npcRoot and (npcRoot.Position - root.Position).Magnitude <= radius then
                pcall(flow.NPCs.Damage, object, object.Health + 1)
            end
        end
    end
end
...
local function toEnd()
    ...
    if not map then return false, "Map não encontrado" end
...
local function current()
    return enabled and not Window.Unloaded
end
        ...
        while current() do
            ...
            if lobbies then
                ...
                if map then
                    ...
                    teleportPoint(Vector3.new(463, 1797, 83540))

                    local finish = os.clock() + 120
                    while current() and os.clock() < finish do
                        killNPCs(300)
                        task.wait(0.2)
                    end
                    ...
                    teleportPoint(Vector3.new(1060, 2287, 83726))
                    task.wait(3)
